...
class Calculator:
    ...
    def _evaluate_infix(self, tokens: list[str]) -> float:
        ...
        for token in tokens:
            #If previous operator takes precedence over current operator, than run calculation. Otherwise, append new token as operator. 
            if token in self.operators:
                ...
                #Add integers to values list
                try:
                    values.append(float(token))
                #Otherwise string is not a valid input for the calculator
                except ValueError:
    ...
    def _apply_operator(self, operators: list[str], values: list[float]) -> None:
        ...
        #Use pop to take the last items of each list and calculate the result
